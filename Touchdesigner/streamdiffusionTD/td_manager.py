...
class TouchDesignerManager:
    ...
    def __init__(self, config: Union[str, Dict[str, Any]], input_mem_name: str, output_mem_name: str, debug_mode: bool = False, osc_reporter = None):
        ...
        # Auto-build TRT preprocessor engines if needed
        self._ensure_preprocessor_engines()
    ...
    def start_streaming(self) -> None:
        ...
        try:
            # Clear stale errors from previous session and send starting state
            if self.osc_reporter:
                self.osc_reporter.send_error('')
    ...
    def _ensure_preprocessor_engines(self) -> None:
        """Auto-build TRT preprocessor engines if config uses them and they're missing."""
        controlnets = self.config.get('controlnets', [])
        if not controlnets:
            return

        # Map preprocessor names to their build functions
        build_registry = {
            'depth_tensorrt': {
                'module': 'streamdiffusion.tools.compile_depth_anything_tensorrt',
                'export_fn': 'export_depth_anything_to_onnx',
                'build_fn': 'build_tensorrt_engine',
                'engine_filename': 'depth_anything_small_518.engine',
                'onnx_filename': 'depth_anything_small_518.onnx',
                'model_size': 'small',
                'resolution': 518,
            },
        }
        ...
        for cn_config in controlnets:
            preprocessor = cn_config.get('preprocessor', '')
            if preprocessor not in build_registry:
                continue

            engine_path = (cn_config.get('preprocessor_params') or {}).get('engine_path', '')
            if not engine_path:
                continue

            if os.path.isfile(engine_path):
                logger.info(f"TRT preprocessor engine exists: {engine_path}")
                continue

            # Engine missing — attempt auto-build
            build_info = build_registry[preprocessor]
            logger.warning(f"TRT preprocessor engine missing: {engine_path}")
            logger.warning(f"Auto-building {preprocessor} engine (this may take a few minutes)...")

            if self.osc_reporter:
                self.osc_reporter.send_error(f"Building {preprocessor} engine...")

            try:
                mod = __import__(build_info['module'], fromlist=[build_info['export_fn'], build_info['build_fn']])
                export_fn = getattr(mod, build_info['export_fn'])
                build_fn = getattr(mod, build_info['build_fn'])

                from pathlib import Path
                engine_dir = Path(engine_path).parent
                engine_dir.mkdir(parents=True, exist_ok=True)

                onnx_path = engine_dir / build_info['onnx_filename']
                engine_file = Path(engine_path)

                # Step 1: Export to ONNX
                logger.warning(f"Step 1/2: Exporting {preprocessor} model to ONNX...")
                if not export_fn(onnx_path, build_info['model_size'], build_info['resolution'], 'cuda'):
                    logger.error(f"ONNX export failed for {preprocessor}")
                    continue

                # Step 2: Build TRT engine
                logger.warning(f"Step 2/2: Building TRT engine for {preprocessor}...")
                if not build_fn(onnx_path, engine_file, build_info['resolution'], fp16=True):
                    logger.error(f"TRT engine build failed for {preprocessor}")
                    continue
                ...
                logger.warning(f"TRT preprocessor engine built: {engine_path} ({engine_file.stat().st_size / 1024 / 1024:.1f} MB)")

                if self.osc_reporter:
                    self.osc_reporter.send_error('')

            except Exception as e:
                logger.error(f"Failed to auto-build {preprocessor} engine: {e}")
                import traceback
                traceback.print_exc()
    ...
    def _streaming_loop(self) -> None:
        ...
        while self.streaming:
            try:
                ...
                # FPS limiting - sleep to cap frame rate if max_fps is set
                if self.max_fps > 0:
    ...
    def _process_ipadapter_frame(self) -> None:
        ...
        if not self.ipadapter_memory:
            # Lazy reconnect — TD may create the shared mem after the subprocess starts
            try:
                self.ipadapter_memory = shared_memory.SharedMemory(name=self.ipadapter_mem_name)
                logger.info(f"Connected to IPAdapter SharedMemory: {self.ipadapter_mem_name}")
            except FileNotFoundError:
                return
