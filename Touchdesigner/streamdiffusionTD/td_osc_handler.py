...
class OSCParameterHandler:
    ...
    def _setup_osc_handlers(self) -> None:
        ...
        # === FPS Limiting ===
        self.dispatcher.map("/max_fps", self._handle_max_fps)
    ...
    def _handle_max_fps(self, address, *args):
        """Handle FPS limit update (0 = unlimited)"""
        max_fps = int(args[0])
        self.manager.max_fps = max_fps
        logger.info(f"FPS limit set to: {max_fps if max_fps > 0 else 'unlimited'}")
