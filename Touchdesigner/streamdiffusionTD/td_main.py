...
class OSCReporter:
    ...
    # === System Info (sent once on startup) ===
    def send_system_info(self) -> None:
        """Send GPU/CUDA/driver/PyTorch info to TD via OSC. Called once after torch is available."""
        try:
            import torch
            self.client.send_message('/system/gpu_name', torch.cuda.get_device_name(0))
            self.client.send_message('/system/cuda_version', str(torch.version.cuda))
            self.client.send_message('/system/pytorch_version', torch.__version__)
        except Exception:
            self.client.send_message('/system/gpu_name', 'unknown')
            self.client.send_message('/system/cuda_version', 'unknown')
            self.client.send_message('/system/pytorch_version', 'unknown')
        try:
            ...
            self.client.send_message('/system/driver_version', result.stdout.strip())
        except Exception:
            self.client.send_message('/system/driver_version', 'unknown')
...
if yaml_config is None:
    print(f"\033[31mERROR: td_config.yaml is empty or malformed at {yaml_config_path}\033[0m")
    print("This usually means the config failed to generate. Restart the stream from TouchDesigner.")
    sys.exit(1)
